...
const renderProcutsInHombres = () =>{

    const filtrado = products.filter(product => product.cat );

    console.log(filtrado);

    productsHombres.innerHTML =filtrado.map(renderCardPruducto).join('');
}
...
const renderPage = () => {
    ...
    renderProcutsInHombres();
    productsHombres.addEventListener('click',addCarrito);
